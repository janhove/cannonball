...
parade <- function(model, full_data = NULL, size = 20) {
  ...
  # Throw error for unsupported models.
  # Better support for lmer() would be nice.
  ...
  if (is.null(full_data)) {
    other_vars <- colnames(d)[-1]
    var_names <- c(other_vars, outcome_var)
    d <- cbind(simulated_data, d)
  ...
  } else if (!(all(colnames(d) %in% colnames(full_data)))) {
    ...
    stop(paste0("The data frame supplied to 'full_data' needs to contains ",
         ...
         paste(shQuote(colnames(d)[!(colnames(d) %in% colnames(full_data))]), collapse = ", "), "\n",
         paste0("Did you specify the correct data frame?"))

  } else if (nrow(d) != nrow(full_data)) {
    # Different datasets?
    ...
    var_names <- c(other_vars, outcome_var)
    d <- cbind(simulated_data, d[, outcome_var], full_data[, other_vars])
  }

  # Rename the columns with the simulated and true outcome variable
  colnames(d)[1:size] <- c(paste("..sample", 1:size))

  # Convert d to the long format. For now, ..sample 'size' contains
  # the true data.
  d <- d |>
    tidyr::pivot_longer(
      tidyselect::starts_with("..sample "), names_to = "..sample", values_to = "outcome"
      )

  # Rename the column with the true and simulated outcomes appropriately
  colnames(d)[ncol(d)] <- outcome_var
  ...
  if (class(model)[1] %in% c("lm", "gam")) {
    d <- d |>
      ...
      # and retain ds variables (using data = .):
      dplyr::do(broom::augment(update(model, data = .), data = .)) |>
      # Add transformed residuals
      dplyr::mutate(.abs_resid = abs(.resid)) |>
      dplyr::mutate(.sqrt_abs_resid = sqrt(abs(.resid))) |>
      # augment() outputs some additional info;
      # only retain variables, fitted values and residuals here
      dplyr::select("..sample", make.names(var_names), ".fitted", ".resid", ".abs_resid", ".sqrt_abs_resid") |>
    ...
    d <- d |>
      ...
      # and retain ds variables:
      dplyr::do(augment_lmer(model, data = .)) |>
      # Remove simulated datasets for which there was a convergence error
      dplyr::filter(convergence_error == 0) |>
      # Add transformed residuals
      dplyr::mutate(.abs_resid = abs(.resid)) |>
      dplyr::mutate(.sqrt_abs_resid = sqrt(abs(.resid))) |>
      # augment() outputs some additional info;
      # only retain variables, fitted values and residuals here
      dplyr::select("..sample", tidyselect::all_of(var_names), ".fitted", ".resid", ".abs_resid", ".sqrt_abs_resid") |>
      dplyr::ungroup()
  }
  ...
  d_sample$.sample2 <- as.character(d_sample$.sample2)
  d <- d |>
    dplyr::left_join(d_sample, by = c("..sample" = ".sample2")) |>
    dplyr::select(-..sample)
  ...
  attr(d, "outcome_var") <- outcome_var
  attr(d, "other_vars") <- other_vars
  attr(d, "predictor_vars") <- colnames(model.frame(model)[-1])

  # Specify that this parade contains the raw data, not summaries
  attr(d, "data_type") <- "raw"

  # Add information about the model class
  attr(d, "model_class") <- class(model)[1]

  # Warning about convergence errors
  unique_samples <- length(unique(d$.sample))
