...
parade_summary <- function(parade, predictors_only = FALSE) {
  ...
  n_outcome_levels <- parade |>
    dplyr::filter(.sample == attr(parade, "position")) |>
    dplyr::select(attr(parade, "outcome_var")) |>
    dplyr::distinct() |>
    nrow()
  ...
  df_summary <- parade |>
    dplyr::group_by(.sample, !!! rlang::syms(grouping_vars)) |>
    dplyr::summarise(.fitted = unique(.fitted),
    ...
    dplyr::mutate(.cell = factor(dplyr::row_number())) |>
    dplyr::ungroup()
